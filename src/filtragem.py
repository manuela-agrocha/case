...
def filtrar_escopo_casa_dos_ventos():
    ...
    logging.info("REQUISITO 2: Filtrando Conjuntos vinculados (Com Normalização Extrema)...")
    
    df_det_filtrado['chave_temporaria'] = (df_det_filtrado['nom_conjuntousina']
                                           .str.upper()
                                           .str.normalize('NFKD') # Separa os acentos das letras
                                           .str.encode('ascii', errors='ignore') # Exclui o que for lixo/corrompido
                                           .str.decode('utf-8')
                                           .str.replace(r'[^A-Z0-9]', '', regex=True)) # Tira espaços e pontuação
